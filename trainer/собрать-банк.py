...
# пометки вида «[условие на картинке: страницы/…]» нужны были, пока картинок не было
ПОМЕТКА = re.compile(r"\s*\[[^\]]*(?:картинк|рисун)[^\]]*\]\s*", re.I)
...
def main():
    ...
    картинки = json.loads(
        (ИСТОЧНИК / "разбор" / "картинки.json").read_text(encoding="utf-8")
    )
    ...
    for в in база:
        ...
        вопрос = {
            ...
            "ty": тип,
            "q": ПОМЕТКА.sub(" ", re.sub(r"\s+\n", "\n", в["текст"])).strip(),
            "o": [],
            "a": [],
            "pairs": [],
            "pool": [],
            "ans": "",
            "img": картинки.get(в["id"], {}).get("условие"),
            "oimg": картинки.get(в["id"], {}).get("варианты", []),
            "e": р["e"],
            "w": [],
            "k": р["k"],
        }
        if в["тип"] == "multichoice":
            вопрос["o"] = [ПОМЕТКА.sub(" ", х["текст"]).strip() for х in в["варианты"]]
